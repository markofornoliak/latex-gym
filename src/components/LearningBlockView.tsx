...
import { CausalSourceOutput } from './CausalSourceOutput';
...
export function LearningBlockView({block}:{block:LearningBlock}){
  ...
  if(block.type==='source-output')return <article className="learning-block learning-block--source-output"><h2>{block.title}</h2><p>{block.body}</p><CausalSourceOutput source={block.code} result={preview}/></article>;
  return null;
}
