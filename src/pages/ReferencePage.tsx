...
import { runtimeReferenceCategories, searchRuntimeReference } from '../data/runtimeCatalog';

export function ReferencePage(){
  const [query,setQuery]=useState('');
  const [category,setCategory]=useState('Все');
  const results=useMemo(()=>searchRuntimeReference(query).filter(entry=>category==='Все'||entry.category===category),[query,category]);
  return <div className="page editorial-page reference-page">
    ...
    <div className="category-strip" role="list" aria-label="Категории справочника">{['Все',...runtimeReferenceCategories].map(item=><button aria-pressed={item===category} className={item===category?'active':''} onClick={()=>setCategory(item)} key={item}>{item}</button>)}</div>
