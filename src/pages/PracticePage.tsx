...
export function PracticePage(){
  const scores=useAppStore(state=>state.conceptScores);const mastery=useAppStore(state=>state.conceptMastery);const mistakes=useAppStore(state=>state.mistakes);const completedLessons=useAppStore(state=>state.completedLessons);const completed=useAppStore(state=>state.completedExercises);
  const daily=selectDailyTraining(exercises,scores,completedLessons,undefined,mastery,mistakes);
  return <div className="page editorial-page"><header className="page-intro"><span className="eyebrow">ПРАКТИКА</span><h1>Тренировка исходника</h1><p>Подборка учитывает изученные темы, прежние ошибки, пробелы в предпосылках и понятия, которые пора повторить. Форматы намеренно чередуются: распознавание не подменяет самостоятельное написание и отладку.</p></header>
