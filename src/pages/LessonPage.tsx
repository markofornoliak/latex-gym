...
export function LessonPage(){
  ...
  useEffect(()=>{if(tab==='example'&&lesson)compiler.compile(lesson.examples[0].code).then(setExampleResult).catch(()=>setExampleResult(null));},[tab,lesson?.id]);
  const index=lesson?lessonIndex.get(lesson.id)??0:0; const prev=lessons[index-1];const next=lessons[index+1];
  const slideCount=tab==='theory'?lesson?.theory.length??1:tab==='example'?1:lesson?.exercises.length??1;
  ...
  return <div className="lesson-page">
    ...
    <div className="lesson-center">
      ...
      <section className="lesson-content">
        ...
        {tab==='example'&&<div className="example-mode"><div><h2>{lesson.examples[0].title}</h2><p>{lesson.examples[0].description}</p><CodeBlock code={lesson.examples[0].code}/></div><div className="example-output"><div className="mode-label">РЕЗУЛЬТАТ</div><LatexPreview result={exampleResult}/></div></div>}
    ...
    <aside className="lesson-context"><span className="eyebrow">КОНТЕКСТ</span><h3>Связанные команды</h3>{lesson.relatedCommands.map(c=><code key={c}>\{c}</code>)}<p>Содержание урока хранится как структурированные данные и не связано с представлением страницы.</p></aside>
  </div>;
}
