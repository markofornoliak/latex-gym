...
export function PracticeExercisePage(){
  const {exerciseId}=useParams();const exercise=getExercise(exerciseId);const settings=useAppStore(s=>s.settings);const drafts=useAppStore(s=>s.drafts);const setDraft=useAppStore(s=>s.setDraft);const recordAttempt=useAppStore(s=>s.recordExerciseAttempt);const recordHint=useAppStore(s=>s.recordHint);const hintsUsed=useAppStore(s=>s.hintsUsed);
  const initial=exercise?(drafts[`exercise:${exercise.id}`]??exercise.starterCode):'';const [source,setSource]=useState(initial);const [result,setResult]=useState<CompileResult|null>(null);const [validation,setValidation]=useState<ValidationResult|null>(null);const [busy,setBusy]=useState(false);const [solution,setSolution]=useState(false);const [mobileTab,setMobileTab]=useState<'editor'|'preview'>('editor');
  ...
  const compile=async()=>{setBusy(true);setValidation(null);try{const r=await compiler.compile(source);setResult(r);setMobileTab('preview');return r;}finally{setBusy(false)}};
  const check=async()=>{setBusy(true);try{const r=await compiler.compile(source);setResult(r);const v=validateExercise(exercise,source,r);setValidation(v);recordAttempt(exercise.id,v.ok,exercise.concepts,exercise.title);if(v.ok)setSolution(true);setMobileTab('preview');}finally{setBusy(false)}};
  ...
  return <div className="practice-screen">
    <header className="practice-top"><Link to="/practice" aria-label="Назад к практике"><BackIcon/></Link><strong>Практика</strong><span className="practice-header-spacer"/></header>
    <div className="practice-workspace">
      <section className="task-pane">
        ...
        <div className="hint-area"><div className="hint-heading"><span>Подсказки</span><button onClick={revealHint} disabled={hintLevel>=exercise.hints.length}>{hintLevel>=exercise.hints.length?'Все открыты':'Открыть подсказку'}</button></div>{exercise.hints.slice(0,hintLevel).map((h,i)=><p key={i}><b>{i+1}.</b> {h}</p>)}{hintLevel>=exercise.hints.length&&!solution&&<button className="text-tool reveal-solution" onClick={()=>setSolution(true)}>Показать эталонное решение</button>}</div>
      </section>
      <section className={`editor-pane ${mobileTab==='editor'?'mobile-active':''}`}>
        <div className="mobile-editor-tabs"><button className={mobileTab==='editor'?'active':''} onClick={()=>setMobileTab('editor')}>Редактор</button><button className={mobileTab==='preview'?'active':''} onClick={()=>setMobileTab('preview')}>Результат</button></div>
        <div className="editor-pane-inner"><Suspense fallback={<div className="editor-loading">Загрузка редактора…</div>}><CodeEditor value={source} onChange={setSource} wordWrap={settings.wordWrap} showLineNumbers={settings.lineNumbers} autoClose={settings.autoClose} minHeight={330} onReset={()=>setSource(exercise.starterCode)}/></Suspense>
          <button className="compile-button" onClick={compile} disabled={busy}><PlayIcon/>{busy?'Компиляция…':'Скомпилировать'}</button>
        </div>
      </section>
      <section className={`result-pane ${mobileTab==='preview'?'mobile-active':''}`}><h2>Результат</h2><div className="result-frame"><LatexPreview result={result}/></div>
