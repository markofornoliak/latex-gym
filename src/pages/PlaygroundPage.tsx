...
import type { CompilationState, CompileResult, CompilerEngine } from '../types';
...
export function PlaygroundPage(){
  ...
  const [source,setSource]=useState(draft??templates['Минимальный документ']);const [result,setResult]=useState<CompileResult|null>(null);const [compileState,setCompileState]=useState<CompilationState>('ready');const [engine,setEngine]=useState<RealEngine>('pdflatex');const [mobileTab,setMobileTab]=useState<'editor'|'preview'>('editor');
  useEffect(()=>{const id=window.setTimeout(()=>setDraft('playground',source),250);return()=>window.clearTimeout(id);},[source,setDraft]);
  const names=useMemo(()=>Object.keys(templates),[]);const capabilities=compiler.getPrimaryCapabilities();const busy=isCompilationBusy(compileState);
  const compile=async()=>{setCompileState('queued');try{const compiled=await compiler.compile(source,{engine,onPhase:setCompileState});setResult(compiled);setMobileTab('preview');}catch(error){const message=error instanceof Error?error.message:String(error);setCompileState('error');setResult({ok:false,diagnostics:[{severity:'error',line:1,message:'Компилятор не завершил запрос',explanation:'Исходник сохранён. Ни основной движок, ни fallback не вернули результат.',suggestion:'Повторите компиляцию после проверки соединения.',source:'latex-gym',originalCompilerMessage:message}],blocks:[],elapsedMs:1,engine:'educational-preview'});setMobileTab('preview');}};
  ...
  return <div className="playground-page"><header className="workspace-heading"><div><span className="eyebrow">PLAYGROUND</span><h1>Свободный документ</h1></div><div className="workspace-actions"><select aria-label="Загрузить шаблон" defaultValue="" onChange={e=>{if(e.target.value)setSource(templates[e.target.value])}}><option value="" disabled>Загрузить шаблон</option>{names.map(n=><option key={n}>{n}</option>)}</select><select aria-label="TeX-движок" value={engine} onChange={event=>setEngine(event.target.value as RealEngine)} disabled={busy}>{capabilities.engines.map(item=><option key={item} value={item}>{engineLabel(item)}</option>)}</select><button className="secondary-button" onClick={download}><DownloadIcon/> .tex</button><button className="primary-button" onClick={compile} disabled={busy}><PlayIcon/>{busy?compilationStateLabel(compileState):'Скомпилировать'}</button></div></header>
    ...
    <div className="playground-workspace"><section className={mobileTab==='editor'?'mobile-active':''}><div className="editor-status-line" aria-live="polite"><span className={`compile-state compile-state--${compileState}`}>{compilationStateLabel(compileState)}</span><span>{result?.fallbackReason?'Учебный fallback':engineLabel(engine)}</span></div><Suspense fallback={<div className="editor-loading">Загрузка редактора…</div>}><CodeEditor value={source} onChange={setSource} wordWrap={settings.wordWrap} showLineNumbers={settings.lineNumbers} autoClose={settings.autoClose} minHeight={620} onReset={()=>setSource(templates['Минимальный документ'])} onCompile={()=>{void compile();}} diagnostics={result?.diagnostics??[]}/></Suspense></section><section className={`playground-preview ${mobileTab==='preview'?'mobile-active':''}`}><div className="mode-label">РЕЗУЛЬТАТ</div><LatexPreview result={result}/></section></div>
  </div>;
}
