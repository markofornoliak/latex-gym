...
const experiences:Array<{id:OnboardingExperience;title:string;detail:string}>=[
  ...
  {id:'regular',title:'Регулярно использую LaTeX',detail:'Сместим placement к диагностике и рабочим паттернам.'},
  {id:'advanced',title:'Уверенно работаю с LaTeX',detail:'Проверим архитектуру, ссылки и компиляционный workflow.'}
];
...
const tasks:PlacementTask[]=[
  ...
  {id:'inline-math',concept:'inline-math',difficulty:1,prompt:'Как оставить формулу частью строки текста?',code:'Energy is ... in this model.',options:[{id:'a',label:'Energy is $E=mc^2$ in this model.'},{id:'b',label:'Energy is \\[E=mc^2\\] in this model.'},{id:'c',label:'Energy is E=mc^2 in this model.'}],answer:'a',explanation:'$...$ задаёт inline math; \\[...\\] создаёт отдельный display-блок.'},
...
function trackForGoals(selected:string[]){
  if(selected.includes('scientific-papers'))return 'Основы LaTeX → Academic Paper';
  if(selected.includes('thesis'))return 'Основы LaTeX → Thesis / Dissertation';
  if(selected.includes('mathematics'))return 'Основы LaTeX → Mathematics';
  if(selected.includes('engineering'))return 'Основы LaTeX → Physics & Engineering';
