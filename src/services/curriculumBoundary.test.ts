...
const forbidden=/from\s+['"][^'"]*\/data\/(courses|concepts|projects|reference|curriculumSource)['"]/g;
const constructionServices=new Set(['./curriculumLinter.ts','./curriculumGraph.ts']);

describe('curriculum runtime boundary',()=>{
  it('keeps runtime application code off canonical and compatibility authoring modules',()=>{
