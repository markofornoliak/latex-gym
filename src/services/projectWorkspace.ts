...
import { activeLatexSource, documentClass } from './latexSourceAnalysis';
import { validateProjectStageContract } from './projectStageValidation';
...
export function projectStageConcepts(projectId:string,stageId:string){return stageSeeds[`${projectId}:${stageId}`]?.concepts??[];}
...
export function assessProjectStage(project:LearningProject,stageIndex:number,workspace:ProjectWorkspace,result:CompileResult):ProjectAssessment{
  ...
  const roots=texFiles.filter(path=>documentClass(workspace.files[path]??'')!==null);
  ...
  for(let index=0;index<=stageIndex;index+=1){
    const stage=project.stages[index];
    if(stage)items.push(...validateProjectStageContract(project.id,stage.id,workspace));
  }
...
function findUnresolvedInputs(workspace:ProjectWorkspace){
  ...
  for(const [sourcePath,source] of Object.entries(workspace.files)){
    ...
    for(const match of activeLatexSource(source).matchAll(/\\(?:input|include)\{([^}]+)\}/g)){
