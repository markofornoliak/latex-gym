...
export type CognitiveSkill='recognition'|'understanding'|'prediction'|'modification'|'debugging'|'production'|'transfer'|'architecture';
export type CompilerRequirement='educational-preview'|'real-tex';
export type ConceptLearningState='unseen'|'introduced'|'understood'|'practiced'|'independent'|'mastered'|'review-due'|'weak';
export type MistakeCategory='environment-balance'|'math-mode'|'semantic-structure'|'package-placement'|'table-alignment'|'paragraph-break'|'superscript-grouping'|'alignment-point'|'reference-placement'|'bibliography-workflow'|'unknown-command'|'file-reference'|'group-balance'|'other';
...
export type StructuralQuery=
 |{target:'fraction';within?:'inlineMath'|'displayMath';numeratorContains?:string;denominatorContains?:string;min?:number}
 |{target:'environment';name:string;containsCommand?:string;min?:number}
 |{target:'command';name:string;argumentContains?:string;withinEnvironment?:string;min?:number}
 |{target:'table';minRows?:number;columns?:number}
 |{target:'labelReference';label?:string};
export type ValidatorRule=
 ...
 |{type:'structure';query:StructuralQuery;message:string;hint:string};

export type Exercise={id:string;lessonId:string;category:PracticeCategory;difficulty:Difficulty;mode:'Написать код'|'Исправить ошибку'|'Предсказать результат'|'Дополнить документ'|'Рефакторинг'|'Найти ошибку'|'Воссоздать результат'|'Текст → LaTeX'|'Улучшить код'|'Собрать документ'|'Объяснить'|'Архитектура';title:string;instructions:string;requirements:string[];starterCode:string;validators:ValidatorRule[];hints:string[];solution:string;concepts:string[];prerequisites?:string[];cognitiveSkill?:CognitiveSkill;compilerRequirement?:CompilerRequirement};
...
export type Diagnostic={severity:'error'|'warning'|'info';line:number;message:string;explanation:string;suggestion?:string;column?:number;rawMessage?:string;conceptId?:string;mistakeCategory?:MistakeCategory;hints?:string[]};
export type PreviewBlock={type:'title';text:string;meta?:string}|{type:'heading';level:1|2|3;text:string}|{type:'paragraph';text:string}|{type:'math';latex:string;display:boolean}|{type:'list';ordered:boolean;items:string[]}|{type:'table';rows:string[][]}|{type:'notice';text:string};
export type CompileResult={ok:boolean;diagnostics:Diagnostic[];blocks:PreviewBlock[];elapsedMs:number;engine:'educational-preview'|'wasm-tex'};
...
export type LearningProject={id:string;title:string;subtitle:string;difficulty:Difficulty;description:string;prerequisites:string[];concepts:string[];stages:Array<{id:string;title:string;objective:string;requirements:string[];starterCode:string;compilerRequirement?:CompilerRequirement}>};
export type ConceptMastery={score:number;attempts:number;successes:number;mistakeCount:number;lastPracticed:string|null;stability:number;nextReview:string|null;firstTrySuccesses?:number;hintedSuccesses?:number;solutionReveals?:number;applications?:number};
export type MistakeEvidence={category:MistakeCategory;count:number;lastSeen:string;exerciseIds:string[];conceptIds:string[]};
