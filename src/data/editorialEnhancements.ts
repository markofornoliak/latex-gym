 ...
 */
export function applyEditorialEnhancements(input:CurriculumDraft):CurriculumDraft{
  const draft=cloneCurriculumDraft(input);
  const firstLesson=draft.lessons.find(lesson=>lesson.id==='document-structure');
  if(firstLesson){
