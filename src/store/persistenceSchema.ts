...
export function parseProgressImport(raw:string):{ok:true;value:ParsedProgressImport}|{ok:false;message:string}{
  ...
  const progress=isRecord(parsed.progress)?parsed.progress:parsed;const projects=isRecord(parsed.projects)?parsed.projects:{};const output:ParsedProgressImport={schemaVersion,documents:{}};
  ...
  const projectProgress=isRecord(projects.completedStages)?projects.completedStages:isRecord(progress.completedProjectStages)?progress.completedProjectStages:null;if(projectProgress)output.completedProjectStages=stringArrayRecord(projectProgress);
  ...
  if(isRecord(progress.attempts))output.attempts=exerciseCounter(progress.attempts,'sum');if(isRecord(progress.successfulAttempts))output.successfulAttempts=exerciseCounter(progress.successfulAttempts,'sum');if(isRecord(progress.hintsUsed))output.hintsUsed=exerciseCounter(progress.hintsUsed,'max');if(isRecord(progress.solutionReveals))output.solutionReveals=exerciseCounter(progress.solutionReveals,'sum');
  if(isRecord(progress.conceptScores))output.conceptScores=numberRecord(progress.conceptScores,true);if(isRecord(progress.conceptMastery))output.conceptMastery=masteryRecord(progress.conceptMastery);if(Array.isArray(progress.history))output.history=sanitizeHistory(progress.history);
  ...
  const documents=isRecord(parsed.documents)?stringRecord(parsed.documents):{};const progressDrafts=isRecord(progress.drafts)?migrateLegacyDraftRecord(stringRecord(progress.drafts)):{};const projectDrafts=isRecord(projects.drafts)?migrateLegacyDraftRecord(stringRecord(projects.drafts)):{};
...
export function sanitizePersistedState(value:unknown):Partial<ParsedProgressImport>{if(!isRecord(value))return {};const parsed=parseProgressImport(JSON.stringify(value));return parsed.ok?parsed.value:{};}
function sanitizeOnboarding(value:Record<string,unknown>):Partial<PersistedOnboarding>{const result:Partial<PersistedOnboarding>={};if(Array.isArray(value.goals))result.goals=stringArray(value.goals);if(value.experience===null||['new','basic','regular','advanced'].includes(String(value.experience)))result.experience=value.experience as PersistedOnboarding['experience'];if(value.placementScore===null||isFiniteNumber(value.placementScore))result.placementScore=value.placementScore===null?null:finiteInteger(value.placementScore,0,0);if(isFiniteNumber(value.placementTotal))result.placementTotal=finiteInteger(value.placementTotal,0,0);if(isRecord(value.placementEvidence))result.placementEvidence=booleanRecord(value.placementEvidence);if(value.recommendedLessonId===null||typeof value.recommendedLessonId==='string')result.recommendedLessonId=value.recommendedLessonId as string|null;if(value.completedAt===null||isDateString(value.completedAt))result.completedAt=value.completedAt as string|null;return result;}
...
function sanitizeBookmarks(values:unknown[]):Bookmark[]{const result:Bookmark[]=[];const seen=new Set<string>();for(const value of values){if(!isRecord(value)||!['lesson','exercise','reference'].includes(String(value.type))||typeof value.targetId!=='string')continue;const type=value.type as Bookmark['type'];const targetId=type==='exercise'?canonicalExerciseId(value.targetId):value.targetId;if(!targetId)continue;const id=`${type}:${targetId}`;if(seen.has(id))continue;seen.add(id);result.push({id,type,targetId,createdAt:isDateString(value.createdAt)?value.createdAt:new Date(0).toISOString()});}return result;}
function sanitizeHistory(values:unknown[]):HistoryEntry[]{const result:HistoryEntry[]=[];for(const value of values){if(!isRecord(value)||typeof value.text!=='string'||!['lesson','exercise','reference'].includes(String(value.kind)))continue;result.push({id:typeof value.id==='string'?value.id:crypto.randomUUID(),at:isDateString(value.at)?value.at:new Date(0).toISOString(),text:value.text,kind:value.kind as HistoryEntry['kind']});if(result.length>=100)break;}return result;}
function exerciseCounter(value:Record<string,unknown>,mode:'sum'|'max'){const raw:Record<string,number>={};for(const [id,count] of Object.entries(value))if(isFiniteNumber(count))raw[id]=finiteInteger(count,0,0);return migrateExerciseKeyedRecord(raw,(left,right)=>mode==='sum'?(left??0)+right:Math.max(left??0,right));}
...
function sanitizeEvidence(value:Record<string,unknown>):MasteryEvidence|null{
  if(!['success','failure'].includes(String(value.outcome)))return null;
  if(!['independent','hinted','revealed'].includes(String(value.independence)))return null;
  if(!['practice','transfer','project','placement'].includes(String(value.context)))return null;
  if(typeof value.realCompile!=='boolean')return null;
  return {outcome:value.outcome as MasteryEvidence['outcome'],independence:value.independence as MasteryEvidence['independence'],context:value.context as MasteryEvidence['context'],realCompile:value.realCompile};
}
