...
import { decodeProjectAsset, isEncodedProjectAsset, PROJECT_ASSET_TOTAL_BYTES } from '../services/projectAssets';
...
export function parseProgressImport(raw:string):{ok:true;value:ParsedProgressImport}|{ok:false;message:string}{return parseProgress(raw,{enforceRawLimit:true});}
...
function parseProgress(raw:string,options:ParseOptions):{ok:true;value:ParsedProgressImport}|{ok:false;message:string}{
  ...
  const boundsProblem=importBoundsProblem(parsed);if(boundsProblem)return {ok:false,message:boundsProblem};
  const schemaVersion=finiteInteger(parsed.schemaVersion,0,0,PERSISTENCE_SCHEMA_VERSION+1);
  ...
  if(isRecord(progress.conceptScores))output.conceptScores=numberRecord(progress.conceptScores,true,IMPORT_LIMITS.maxConceptScore);
  ...
  if(isRecord(progress.streak))output.streak={count:finiteInteger(progress.streak.count,0,0,IMPORT_LIMITS.maxCounter),lastActive:nullableDate(progress.streak.lastActive)};
  const settings=isRecord(parsed.settings)?parsed.settings:isRecord(progress.settings)?progress.settings:null;if(settings)output.settings=sanitizeSettings(settings);
  ...
  for(const [key,value] of Object.entries({...progressDrafts,...projectDrafts,...documents})){
    const migrated=migrateDocumentKey(key);if(safeRecordKey(migrated))output.documents[migrated]=value;
  }
...
export function sanitizePersistedState(value:unknown):Partial<ParsedProgressImport>{
  ...
  let raw:string;try{raw=JSON.stringify(value);}catch{return {};}
  const parsed=parseProgress(raw,{enforceRawLimit:false});return parsed.ok?parsed.value:{};
}
...
function importBoundsProblem(root:Record<string,unknown>):string|null{
  ...
  const onboarding=isRecord(progress.onboarding)?progress.onboarding:{};
  const boundedRecords=[progress.attempts,progress.successfulAttempts,progress.hintsUsed,progress.solutionReveals,progress.conceptScores,progress.conceptMastery,onboarding.placementEvidence,progress.completedProjectStages,projects.completedStages];
  ...
  const boundedArrays=[progress.completedLessons,progress.completedExercises,progress.bookmarks,progress.history,onboarding.goals];
  ...
  let documentCount=0,totalChars=0,binaryBytes=0;
  for(const source of documentSources){
    for(const [key,value] of Object.entries(source)){
      documentCount++;if(documentCount>IMPORT_LIMITS.maxDocuments)return 'Файл прогресса содержит слишком много локальных документов.';
      ...
      totalChars+=value.length;if(totalChars>IMPORT_LIMITS.maxDocumentTotalChars)return 'Суммарный объём локальных документов слишком большой для безопасного импорта.';
      if(isEncodedProjectAsset(value)){
        const decoded=decodeProjectAsset(value);if(!decoded)return `Бинарный файл «${key.slice(0,80)}» повреждён или превышает безопасный размер.`;
        binaryBytes+=decoded.length;if(binaryBytes>PROJECT_ASSET_TOTAL_BYTES)return 'Суммарный объём бинарных файлов проекта превышает безопасный локальный лимит.';
      }
    }
  }
  return null;
...
function sanitizeOnboarding(value:Record<string,unknown>):Partial<PersistedOnboarding>{
  ...
  if(value.placementScore===null||isFiniteNumber(value.placementScore))result.placementScore=value.placementScore===null?null:finiteInteger(value.placementScore,0,0,IMPORT_LIMITS.maxCounter);
  if(isFiniteNumber(value.placementTotal))result.placementTotal=finiteInteger(value.placementTotal,0,0,IMPORT_LIMITS.maxCounter);
  ...
  if(result.placementScore!==null&&result.placementScore!==undefined&&result.placementTotal!==undefined)result.placementScore=Math.min(result.placementScore,result.placementTotal);
  return result;
}
...
function exerciseCounter(value:Record<string,unknown>,mode:'sum'|'max'){
  const raw:Record<string,number>={};for(const [id,count] of Object.entries(value)){if(!safeRecordKey(id)||!isFiniteNumber(count))continue;raw[id]=finiteInteger(count,0,0,IMPORT_LIMITS.maxCounter);}
  const migrated=migrateExerciseKeyedRecord(raw,(left,right)=>mode==='sum'?saturatingAdd(left??0,right,IMPORT_LIMITS.maxCounter):Math.min(IMPORT_LIMITS.maxCounter,Math.max(left??0,right)));
  return numberRecord(migrated,false,IMPORT_LIMITS.maxCounter);
}
...
function sanitizeMastery(value:Record<string,unknown>):Partial<ConceptMastery>{
  ...
  if(isFiniteNumber(value.attempts))out.attempts=finiteInteger(value.attempts,0,0,IMPORT_LIMITS.maxCounter);
  if(isFiniteNumber(value.successes))out.successes=finiteInteger(value.successes,0,0,IMPORT_LIMITS.maxCounter);
  if(isFiniteNumber(value.mistakeCount))out.mistakeCount=finiteInteger(value.mistakeCount,0,0,IMPORT_LIMITS.maxCounter);
  if(isFiniteNumber(value.stability))out.stability=clamp(value.stability,.5,IMPORT_LIMITS.maxStability);
  if(isFiniteNumber(value.independentSuccesses))out.independentSuccesses=finiteInteger(value.independentSuccesses,0,0,IMPORT_LIMITS.maxCounter);
  if(isFiniteNumber(value.hintedSuccesses))out.hintedSuccesses=finiteInteger(value.hintedSuccesses,0,0,IMPORT_LIMITS.maxCounter);
  if(isFiniteNumber(value.transferSuccesses))out.transferSuccesses=finiteInteger(value.transferSuccesses,0,0,IMPORT_LIMITS.maxCounter);
  if(isFiniteNumber(value.projectSuccesses))out.projectSuccesses=finiteInteger(value.projectSuccesses,0,0,IMPORT_LIMITS.maxCounter);
  if(isFiniteNumber(value.solutionReveals))out.solutionReveals=finiteInteger(value.solutionReveals,0,0,IMPORT_LIMITS.maxCounter);
  if(isFiniteNumber(value.delayedRecallSuccesses))out.delayedRecallSuccesses=finiteInteger(value.delayedRecallSuccesses,0,0,IMPORT_LIMITS.maxCounter);
  if(value.lastSuccessfulDelayDays===null||isFiniteNumber(value.lastSuccessfulDelayDays))out.lastSuccessfulDelayDays=value.lastSuccessfulDelayDays===null?null:clamp(Number(value.lastSuccessfulDelayDays),0,IMPORT_LIMITS.maxDelayDays);
  ...
  if(out.successes!==undefined&&out.attempts!==undefined)out.successes=Math.min(out.successes,out.attempts);
  if(out.mistakeCount!==undefined&&out.attempts!==undefined)out.mistakeCount=Math.min(out.mistakeCount,out.attempts);
  return out;
}
...
function numberRecord(value:Record<string,unknown>,allowNegative=false,max=IMPORT_LIMITS.maxCounter){const result:Record<string,number>={};for(const [key,item] of Object.entries(value)){if(!safeRecordKey(key)||!isFiniteNumber(item)||(item<0&&!allowNegative))continue;result[key]=clamp(Number(item),allowNegative?-max:0,max);}return result;}
...
function finiteInteger(value:unknown,fallback=0,min=Number.MIN_SAFE_INTEGER,max=Number.MAX_SAFE_INTEGER){return isFiniteNumber(value)?clamp(Math.trunc(value),min,max):fallback;}
function saturatingAdd(left:number,right:number,max:number){const sum=left+right;return Number.isFinite(sum)?Math.min(max,sum):max;}
